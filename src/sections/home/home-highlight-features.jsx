...
function ScrollContent({ containerRoot }) {
  ...
  // Calculate scrollRange with safety checks
  const scrollRange = scrollRect.scrollWidth > 0 && containeRect.width > 0
    ? -scrollRect.scrollWidth + containeRect.width / 2
    : 0;

  // Only create transforms when we have valid dimensions
  const x = useSpring(
    useTransform(scrollYProgress, [0, 1], [0, scrollRange]),
    physics
  );

  // Ensure input and output arrays have the same length (9 values each)
  const inputRange = [0, 0.12, 0.28, 0.48, 0.58, 0.62, 0.72, 0.92, 1];
  const outputRange = [
    `transparent`,
    ...
    `linear-gradient(180deg, ${PRIMARY_COLOR.brown.light}, ${PRIMARY_COLOR.brown.dark})`,
    `linear-gradient(180deg, ${PRIMARY_COLOR['ai-nexus'].light}, ${PRIMARY_COLOR['ai-nexus'].dark})`,
    `linear-gradient(180deg, ${PRIMARY_COLOR['ai-nexus-yellow'].light}, ${PRIMARY_COLOR['ai-nexus-yellow'].dark})`,
