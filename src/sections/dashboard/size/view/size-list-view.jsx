...
import { fetchSizes, deleteSize } from 'src/store/slices/sizeSlice';
...
export function SizeListView() {
  ...
  const { sizes: tableData, loading } = useSelector((state) => state.sizes);
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      const deletePromises = table.selected.map((id) => dispatch(deleteSize(id)).unwrap());
      ...
      toast.error(error || 'Failed to delete sizes');
    }
  }, [dataFiltered.length, dataInPage.length, dispatch, table]);

  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.admin.size.edit(id));
    },
    [router]
  );
  ...
  return (
    <>
      <DashboardContent>
        ...
        <Card>
          <SizeTableToolbar filters={filters} onResetPage={table.onResetPage} />
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (size) =>
        size.title.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }
