...
export function OverviewAppView() {
  ...
  // Calculate total active users dynamically
  const totalActiveUsers = useMemo(() => {
    if (!users || users.length === 0) return 0;
    return users.filter((u) => u.status === 'Active').length;
  }, [users]);

  // Calculate percentage change (simple calculation - can be enhanced with date comparison)
  // For now, return a simple positive value
  // In future, you can compare with previous week/month data
  const percentChange = useMemo(() => (totalActiveUsers > 0 ? 2.6 : 0), [totalActiveUsers]);
